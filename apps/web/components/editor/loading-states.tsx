    ...
    return (
        <div className="space-y-3">
            {Array.from({ length: count }).map((_, i) => (
                <div
                    key={i}
                    className="rounded-lg border border-white/10 backdrop-blur-sm bg-black/20 p-4 animate-pulse"
                >
                    {/* Preview block (same place/size as real card preview) */}
                    <div className="w-full h-28 rounded-md border border-white/10 bg-white/5 mb-3" />
