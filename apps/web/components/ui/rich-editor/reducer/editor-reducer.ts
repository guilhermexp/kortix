...
export function editorReducer(
  ...
  const getCurrentContainer = (): ContainerNode => {
    return state.history[state.historyIndex] ?? state.history[0]!;
  };
  ...
  switch (action.type) {
    ...
    case "SWAP_NODES": {
      ...
      const node1 = newChildren[index1];
      const node2 = newChildren[index2];
      if (!node1 || !node2) return state;

      [newChildren[index1], newChildren[index2]] = [node2, node1];
