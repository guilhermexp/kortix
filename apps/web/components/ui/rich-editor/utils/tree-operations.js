...
exports.validateTree = exports.traverseTree = exports.cloneNode = exports.moveNode = exports.insertNode = exports.deleteNodeById = exports.updateNodeById = exports.findParentById = exports.findNodeById = void 0;
...
function findNodeById(node, targetId) {
    ...
    if ((0, types_1.isContainerNode)(node) || (0, types_1.isStructuralNode)(node)) {
...
function findParentById(node, targetId) {
    ...
    for (var _b = 0, _c = node.children; _b < _c.length; _b++) {
        var child = _c[_b];
        if ((0, types_1.isContainerNode)(child) || (0, types_1.isStructuralNode)(child)) {
...
function updateNodeById(node, targetId, updater) {
    ...
    if ((0, types_1.isContainerNode)(node) || (0, types_1.isStructuralNode)(node)) {
        ...
        var childrenChanged = newChildren.some(function (newChild, index) { return newChild !== node.children[index]; });
        if (childrenChanged) {
...
function deleteNodeById(node, targetId) {
    ...
    if ((0, types_1.isContainerNode)(node) || (0, types_1.isStructuralNode)(node)) {
        var newChildren = node.children
            .map(function (child) { return deleteNodeById(child, targetId); })
...
function insertNode(root, targetId, newNode, position) {
    ...
    if (position === "prepend" || position === "append") {
        return updateNodeById(root, targetId, function (node) {
            if (!(0, types_1.isContainerNode)(node) && !(0, types_1.isStructuralNode)(node)) {
                console.warn("Cannot ".concat(position, " to non-container/structural node ").concat(targetId));
                return {};
            }
            return {
                children: position === "prepend"
                    ? __spreadArray([newNode], node.children, true) : __spreadArray(__spreadArray([], node.children, true), [newNode], false)
            };
        });
    }
...
function insertNodeRecursive(node, targetId, newNode, position) {
    ...
    // Check if target is a direct child
    var targetIndex = node.children.findIndex(function (child) { return child.id === targetId; });
    if (targetIndex !== -1) {
    ...
    // Recursively search in children
    var newChildren = node.children.map(function (child) {
        return insertNodeRecursive(child, targetId, newNode, position);
    });
    // Only create new object if children changed
    var childrenChanged = newChildren.some(function (newChild, index) { return newChild !== node.children[index]; });
    if (childrenChanged) {
...
function cloneNode(node, newId) {
    var cloned = __assign(__assign({}, node), { id: newId || "".concat(node.id, "-clone-").concat(Date.now()) });
    if (((0, types_1.isContainerNode)(cloned) || (0, types_1.isStructuralNode)(cloned)) &&
        ((0, types_1.isContainerNode)(node) || (0, types_1.isStructuralNode)(node))) {
        cloned.children = node.children.map(function (child) { return cloneNode(child); });
    }
    return cloned;
}
...
function validateTree(node) {
    var errors = [];
    var seenIds = new Set();
    traverseTree(node, function (currentNode) {
        ...
        if (((0, types_1.isContainerNode)(currentNode) || (0, types_1.isStructuralNode)(currentNode)) &&
            !Array.isArray(currentNode.children)) {
            errors.push("Container/Structural node ".concat(currentNode.id, " has invalid children property"));
        }
    });
    return {
