...
exports.serializeToHtmlWithClass = exports.serializeToHtmlFragment = exports.serializeToHtml = void 0;
...
function serializeInlineChildren(node) {
    ...
    return node
        ...
        var formattingClasses = getInlineFormattingClasses(child.bold, child.italic, child.underline);
        ...
        if (child.href) {
            ...
            return "<a href=\"".concat(escapeHtml(child.href), "\" target=\"_blank\" rel=\"noopener noreferrer\" class=\"").concat(finalClasses, "\"").concat(styleAttr, ">").concat(content, "</a>");
        }
        if (allClasses || inlineStyles) {
            // Add inline-block pr-1 for italic text to prevent overlapping
            var italicSpacing = child.italic ? "inline-block pr-1" : "";
            var finalClasses = [allClasses, italicSpacing]
                .filter(Boolean)
                .join(" ");
            var classAttr = finalClasses ? " class=\"".concat(finalClasses, "\"") : "";
            var styleAttr = inlineStyles ? " style=\"".concat(inlineStyles, "\"") : "";
            return "<span".concat(classAttr).concat(styleAttr, ">").concat(content, "</span>");
        }
        return content;
    })
        .join("");
...
function serializeTextNode(node, indent) {
    ...
    var type = node.type, attributes = node.attributes;
    // Handle BR elements
    if (type === "br") {
    ...
    // Handle image nodes
    if (type === "img") {
        var src = (attributes === null || attributes === void 0 ? void 0 : attributes.src) || "";
        var alt = (attributes === null || attributes === void 0 ? void 0 : attributes.alt) || "";
        var caption = node.content || "";
        var html = "".concat(indent, "<figure class=\"mb-4\">\n");
        html += "".concat(indent, "  <img src=\"").concat(escapeHtml(src), "\" alt=\"").concat(escapeHtml(alt), "\" class=\"h-auto rounded-lg object-cover max-h-[600px]\" style=\"width: auto; margin: auto;\" />\n");
        if (caption) {
            html += "".concat(indent, "  <figcaption class=\"text-sm text-muted-foreground text-center mt-3 italic\">").concat(escapeHtml(caption), "</figcaption>\n");
        }
    ...
    var customClassName = (attributes === null || attributes === void 0 ? void 0 : attributes.className) || "";
    // Check if className is a hex color (starts with #)
    var isHexColor = typeof customClassName === "string" && customClassName.startsWith("#");
    ...
    var backgroundColor = attributes === null || attributes === void 0 ? void 0 : attributes.backgroundColor;
    // Build inline styles
    var styles = [];
    if (backgroundColor) {
    ...
    return "".concat(indent, "<").concat(tag).concat(classAttr).concat(styleAttr, ">").concat(content, "</").concat(tag, ">\n");
}
/**
...
function serializeTableNode(node, indent) {
    ...
    var tag = node.type;
    if (tag === "table") {
        var html = "".concat(indent, "<table class=\"border-collapse border border-border w-full\">\n");
        ...
        for (var _f = 0, _g = node.children; _f < _g.length; _f++) {
            ...
            if ((0, types_1.isTextNode)(child)) {
                ...
                var cellClass = cellTag === "th"
                    ? "border border-border bg-muted/50 p-2 font-semibold text-left min-w-[100px]"
                    : "border border-border p-2";
                html += "".concat(indent, "  <").concat(cellTag, " class=\"").concat(cellClass, "\">").concat(content, "</").concat(cellTag, ">\n");
            }
        }
...
function serializeContainerNode(node, indent) {
    ...
    var isTableWrapper = (firstChild === null || firstChild === void 0 ? void 0 : firstChild.type) === "table";
    ...
    var layoutType = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.layoutType;
    var isFlexContainer = layoutType === "flex";
    var gap = (_b = node.attributes) === null || _b === void 0 ? void 0 : _b.gap;
    var flexWrap = (_c = node.attributes) === null || _c === void 0 ? void 0 : _c.flexWrap;
    // Determine container type and classes
    var listTypeFromAttribute = (_d = node.attributes) === null || _d === void 0 ? void 0 : _d.listType;
    var listType = listTypeFromAttribute ||
        (firstChild &&
    ...
    // Get custom className from attributes
    var customClassName = ((_e = node.attributes) === null || _e === void 0 ? void 0 : _e.className) || "";
    // Build container classes matching the preview
    var containerClasses = isFlexContainer
        ? "flex flex-row gap-".concat(gap || "4", " items-start ").concat(flexWrap === "wrap" ? "flex-wrap items-center" : "")
        : isListContainer
            ? "list-none pl-0 ml-6"
            : "nested-container border-l-2 border-border/50 pl-4 ml-2";
    // Add custom classes
    if (customClassName) {
        containerClasses = "".concat(containerClasses, " ").concat(customClassName).trim();
    }
    // Get backgroundColor from attributes
    var backgroundColor = (_f = node.attributes) === null || _f === void 0 ? void 0 : _f.backgroundColor;
    // Build inline styles
    var styles = [];
    if (backgroundColor) {
        styles.push("background-color: ".concat(backgroundColor));
    ...
    var html = "".concat(indent, "<").concat(containerTag, " class=\"").concat(containerClasses, "\"").concat(styleAttr, ">\n");
    ...
    while (i < node.children.length) {
        ...
        if ((0, types_1.isTextNode)(child)) {
            ...
            if (isFlexContainer) {
                html += "".concat(indent, "  <div class=\"flex-1 min-w-[280px] max-w-full\">\n");
            ...
            // Check if this is the start of a list (and not already in a list container)
                ...
                // Start ordered list
                html += "".concat(indent, "  <ol class=\"list-decimal list-inside space-y-1\">\n");
                ...
                while (i < node.children.length) {
                    ...
                    if ((0, types_1.isTextNode)(listItem) && listItem.type === "li") {
                        ...
                        if (!isEmpty) {
                            ...
                            html += "".concat(liIndent, "<li class=\"").concat(liClasses, "\">").concat(content, "</li>\n");
                        }
                        i++;
                    }
                    else {
...
function serializeToHtml(container, options) {
    ...
    var _a = options.wrapperClass, wrapperClass = _a === void 0 ? "editor-content" : _a, _b = options.includeWrapper, includeWrapper = _b === void 0 ? true : _b, _c = options.indent, indent = _c === void 0 ? "  " : _c;
    var html = "";
    ...
    while (i < container.children.length) {
        ...
        if ((0, types_1.isTextNode)(child)) {
            ...
            if (textNode.type === "li") {
                // Start ordered list
                html += "".concat(includeWrapper ? indent : "", "<ol class=\"list-decimal list-inside space-y-1\">\n");
                // Add all consecutive list items
                while (i < container.children.length) {
