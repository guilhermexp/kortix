    ...
    try {
        ...
        for (var i = 0; i < bodyLines.length; i++) {
            ...
            if (cells.length !== numCols) {
                return {
                    success: false,
                    error: "Row ".concat(i + 3, " has ").concat(cells.length, " columns, expected ").concat(numCols)
                };
            }
        }
        var timestamp_1 = Date.now();
        // Create header cells
        var headerCellNodes = headerCells.map(function (content, idx) { return ({
        ...
        // Create body rows
        var bodyRows = bodyLines.map(function (line, rowIdx) {
            var cells = line
                .split("|")
                .slice(1, -1)
                .map(function (cell) { return cell.trim(); });
            var cellNodes = cells.map(function (content, colIdx) { return ({
