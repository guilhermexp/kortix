...
exports.restoreSelection = exports.findNodeAnywhere = exports.findNodeInTree = exports.detectFormatsInRange = exports.parseDOMToInlineChildren = void 0;
...
function parseDOMToInlineChildren(element) {
    var children = [];
    var walkNode = function (node, inheritedFormats) {
        if (inheritedFormats === void 0) { inheritedFormats = {}; }
        if (node.nodeType === Node.TEXT_NODE) {
            // Direct text node - use inherited formatting
            var content = node.textContent || "";
            if (content) {
                var hasAnyFormatting = inheritedFormats.bold ||
            ...
            if (classList.some(function (c) { return c.includes("text-4xl"); })) {
                elementType = "h1";
            }
            else if (classList.some(function (c) { return c.includes("text-3xl"); })) {
                elementType = "h2";
            }
            else if (classList.some(function (c) { return c.includes("text-2xl"); })) {
                elementType = "h3";
            }
            else if (classList.some(function (c) { return c.includes("text-xl"); })) {
                elementType = "h4";
            }
            else if (classList.some(function (c) { return c.includes("text-lg"); }) &&
                classList.includes("font-semibold")) {
                elementType = "h5";
            }
            else if (classList.some(function (c) { return c.includes("text-base"); }) &&
                classList.includes("font-semibold")) {
                elementType = "h6";
            }
                ...
                classList.some(function (c) { return c.includes("leading-relaxed"); })) {
                elementType = "p";
            }
            ...
            var customClasses = classList.filter(function (c) { return !knownClasses_1.includes(c); });
            var customClassName = customClasses.length > 0 ? customClasses.join(" ") : undefined;
    ...
    return children.filter(function (child) { return child.content && child.content.length > 0; });
}
...
function detectFormatsInRange(node, start, end) {
    ...
    if (!node.children || node.children.length === 0) {
        return {
            bold: ((_a = node.attributes) === null || _a === void 0 ? void 0 : _a.bold) === true,
            italic: ((_b = node.attributes) === null || _b === void 0 ? void 0 : _b.italic) === true,
            underline: ((_c = node.attributes) === null || _c === void 0 ? void 0 : _c.underline) === true,
            elementType: null,
            href: null,
            className: null,
            styles: null
    ...
    var charsInRange = 0;
    var firstElementType = undefined;
    var allSameElementType = true;
    var firstHref = undefined;
    var allSameHref = true;
    var firstClassName = undefined;
    var allSameClassName = true;
    var firstStyles = undefined;
...
function findNodeAnywhere(id, container) {
    // Check root level
    var rootNode = container.children.find(function (n) { return n.id === id; });
    if (rootNode)
    ...
    for (var _i = 0, _a = container.children; _i < _a.length; _i++) {
        ...
        if ((0, types_1.isContainerNode)(child)) {
            ...
            var foundInContainer = containerNode.children.find(function (c) { return c.id === id; });
            if (foundInContainer)
                return {
...
function restoreSelection(element, start, end) {
    ...
    var walk = function (node) {
        ...
        if (found)
            return;
        if (node.nodeType === Node.TEXT_NODE) {
            var textLength = ((_a = node.textContent) === null || _a === void 0 ? void 0 : _a.length) || 0;
    ...
    if (startNode && endNode && sel) {
        try {
            var startLength = ((_a = startNode.textContent) === null || _a === void 0 ? void 0 : _a.length) || 0;
            var endLength = ((_b = endNode.textContent) === null || _b === void 0 ? void 0 : _b.length) || 0;
