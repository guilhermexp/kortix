...
exports.createHandleDrop =
	exports.createHandleDragLeave =
	exports.createHandleDragOver =
	exports.createHandleDragEnter =
	exports.createHandleBlockDragStart =
	exports.createHandleImageDragStart =
		void 0
...
function createHandleDragOver(params) {
	return (e, nodeId) => {
		...
		var draggingNode =
			draggingResult === null || draggingResult === void 0
				? void 0
				: draggingResult.node
		var isTargetImage =
			(0, types_1.isTextNode)(targetNode) && targetNode.type === "img"
		var isDraggingImage =
			draggingNode &&
		...
		if (isTargetImage && isDraggingImage) {
			...
			if (inSameFlexContainer) {
				...
				var parent_1 = (
					...
					: null
				if (parent_1) {
					var dragIndex = parent_1.children.findIndex(
						(c) => c.id === draggingNodeId,
					)
					var targetIndex = parent_1.children.findIndex((c) => c.id === nodeId)
		...
		if (draggingNodeId) {
			...
			var draggedIndex = container.children.findIndex(
				(n) => n.id === draggingNodeId,
			)
			var targetIndex = container.children.findIndex((n) => n.id === nodeId)
				...
				switch (_e.label) {
					case 0:
						;(container = params.container),
							...
							(toast = params.toast),
							(draggingNodeId = params.draggingNodeId),
							(setDraggingNodeId = params.setDraggingNodeId),
							(setDragOverNodeId = params.setDragOverNodeId),
							(setDropPosition = params.setDropPosition),
						...
						if (draggedNodeId && draggingNodeId) {
							...
							draggingResult = (0, editor_helpers_1.findNodeAnywhere)(
								draggingNodeId,
								container,
							)
							targetResult = (0, editor_helpers_1.findNodeAnywhere)(
								nodeId,
								container,
							)
							...
							if (dropPosition === "left" || dropPosition === "right") {
								...
								if (
									...
									dragIndex = newChildren.findIndex(
										(c) => c.id === draggingNodeId,
									)
									targetIndex = newChildren.findIndex((c) => c.id === nodeId)
									draggedItem = newChildren.splice(dragIndex, 1)[0]
									// Only proceed if draggedItem exists
									if (!draggedItem) return [2 /*return*/]
									adjustedTargetIndex =
									...
									dispatch(
										actions_1.EditorActions.updateNode(parent_2.id, {
								...
								if (
									...
									if (
										...
										targetResult.parentId &&
										((_d =
											(_c = targetResult.parent) === null || _c === void 0
												? void 0
										...
										targetIndex = parent_3.children.findIndex(
											(c) => c.id === nodeId,
										)
									...
									targetRootIndex = container.children.findIndex(
										(n) =>
											n.id === nodeId ||
											((0, types_1.isContainerNode)(n) &&
												n.children.some((c) => c.id === nodeId)),
									)
									draggingRootIndex = container.children.findIndex(
										(n) =>
											n.id === draggingNodeId ||
											((0, types_1.isContainerNode)(n) &&
							...
							if (draggingResult.parentId && draggingResult.parent) {
								...
								remainingChildren = parent_4.children.filter(
									(c) => c.id !== draggingNodeId,
								)
								console.log(
									"  Parent children count:",
									parent_4.children.length,
								)
								console.log(
								...
								if (remainingChildren.length === 1) {
									console.log(
										"  ⚠️ Only 1 child remaining - unwrapping container",
									)
									parentIndex = container.children.findIndex(
										(c) => c.id === parent_4.id,
									)
									...
									if (isTargetTheFlexContainer) {
										...
										if (referenceNodeId) {
											...
											if (!remainingChild) return [2 /*return*/]
											console.log(
											...
											console.log(
												"  Action 3: Delete flex container:",
												parent_4.id,
											)
											actions.push(
												actions_1.EditorActions.deleteNode(parent_4.id),
											)
										} else {
											fallbackChild = remainingChildren[0]
											if (!fallbackChild) return [2 /*return*/]
											console.log(
												"  No siblings found - using container as reference",
											)
											actions.push(
												actions_1.EditorActions.insertNode(
													fallbackChild,
										...
										targetIndex = container.children.findIndex(
											(c) => c.id === nodeId,
										)
										isTargetBeforeFlex =
										...
										if (isTargetBeforeFlex || isTargetAfterFlex) {
											...
											if (!adjacentChild) return [2 /*return*/]
											...
											if (!standardChild) return [2 /*return*/]
											if (parentIndex > 0) {
												prevNode = container.children[parentIndex - 1]
												if (prevNode) {
													console.log(
												...
												console.log(
													"  Action 3: Append remaining child to container",
												)
												actions.push(
													actions_1.EditorActions.insertNode(
														standardChild,
											...
											console.log(
												"  Action 4: Delete flex container:",
												parent_4.id,
											)
											actions.push(
												actions_1.EditorActions.deleteNode(parent_4.id),
									...
									console.log(
										"  ✓ Multiple children remain - updating container",
									)
									// Multiple children remain, just update the flex container
									actions.push(
										actions_1.EditorActions.updateNode(parent_4.id, {
								...
								actions.push(
									actions_1.EditorActions.setActiveNode(draggingNodeId),
								)
								dispatch(actions_1.EditorActions.batch(actions))
								toast({
									title: "Image moved!",
							...
							draggingNodeAtRoot = container.children.find(
								(n) => n.id === draggingNodeId,
							)
							if (draggingNodeAtRoot) {
								targetNodeAtRoot = container.children.find(
									(n) => n.id === nodeId,
								)
								isDraggingImage =
									(0, types_1.isTextNode)(draggingNode) &&
								...
								if (!isDraggingImage && !isTargetImage && targetNodeAtRoot) {
									dispatch(
										actions_1.EditorActions.swapNodes(draggingNodeId, nodeId),
									)
									dispatch(
										actions_1.EditorActions.setActiveNode(draggingNodeId),
									)
									...
									insertPos =
										dropPosition === "before" || dropPosition === "after"
											...
											: "after"
									dispatch(
										actions_1.EditorActions.moveNode(
											draggingNodeId,
											nodeId,
											insertPos,
										),
									...
									toast({
										...
										description: ""
											.concat(
												isDraggingImage ? "Image" : "Block",
												" repositioned ",
											)
											.concat(dropPosition, " the block"),
									})
								}
							}
							setDragOverNodeId(null)
					...
					case 2:
						...
						return [3 /*break*/, 5]
					case 3:
						return [4 /*yield*/, (0, image_upload_1.uploadImage)(mediaFile)]
					case 4:
						result = _e.sent()
