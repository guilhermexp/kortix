	...
	return () => {
		...
		for (const child of container.children) {
			if (selectedImageIds.has(child.id)) {
				...
				if (index > 0) {
					const prevNode = container.children[index - 1]
					if (prevNode) {
			...
			if (flexIndex > 0) {
				...
				if (remainingChildren.length === 1) {
					...
					if (remainingChild && prevNode) {
						actions.push(
							EditorActions.insertNode(remainingChild, prevNode.id, "after"),
						)
					}
				}

				// Insert extracted images
				const firstRemaining = remainingChildren[0]
				const lastNodeId =
				...
				let currentNodeId: string = lastNodeId
				imagesToExtract.forEach((image) => {
					actions.push(EditorActions.insertNode(image, currentNodeId, "after"))
					currentNodeId = image.id
				})
