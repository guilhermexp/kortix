...
exports.createHandleCopyJson =
	exports.createHandleCopyHtml =
	exports.createHandleCreateTable =
	exports.createHandleCreateLink =
	exports.createHandleCreateListFromCommand =
	exports.createHandleCreateList =
	exports.createHandleInsertImageFromCommand =
	exports.createHandleChangeBlockType =
	exports.createHandleCreateNested =
	exports.createHandleAddBlock =
	exports.createHandleDeleteNode =
	exports.createHandleNodeClick =
		void 0
...
function createHandleDeleteNode(params) {
	return (nodeId) => {
		...
		if (parentContainer) {
			...
			var remainingChildren = containerNode.children.filter(
				(c) => c.id !== nodeId,
			)
			...
			if (remainingChildren.length === 1) {
				...
				var containerIndex = container.children.findIndex(
...
function createHandleCreateNested(params) {
	return (nodeId) => {
		...
		if (isAlreadyNested) {
			...
			dispatch(
				actions_1.EditorActions.insertNode(newParagraph_1, nodeId, "after"),
			)
		...
		var nodeIndex = container.children.findIndex((n) => n.id === nodeId)
		if (nodeIndex > 0) {
...
function createHandleChangeBlockType(params) {
	return (nodeId, newType) => {
		...
		if (newType === "li") {
			dispatch(
				actions_1.EditorActions.updateNode(nodeId, {
					type: newType,
					content: "",
				}),
			)
...
function createHandleInsertImageFromCommand(params, fileInputRef) {
	return (nodeId) => {
		...
		setTimeout(() => {
			var _a
			;(_a = fileInputRef.current) === null || _a === void 0
				? void 0
				: _a.click()
		}, 100)
	}
...
function createHandleCreateList(params) {
	return (listType) => {
		...
		if (lastNode) {
			dispatch(
				actions_1.EditorActions.insertNode(listContainer, lastNode.id, "after"),
			)
		} else {
			// If no nodes exist, replace the container
			dispatch(
				actions_1.EditorActions.replaceContainer(
					__assign(__assign({}, container), { children: [listContainer] }),
				),
			)
		}
		...
		toast({
			title: "List Created",
			description: "Added a new ".concat(
				listTypeLabel,
				" list with header and 3 items",
			),
		})
		...
		setTimeout(() => {
			...
			if (editorContent) {
				var lastChild =
					(_a = editorContent.querySelector("[data-editor-content]")) ===
						null || _a === void 0
						? void 0
						: _a.lastElementChild
				if (lastChild) {
...
function createHandleCreateLink(params) {
	return () => {
		...
		if (lastNode) {
			dispatch(
				actions_1.EditorActions.insertNode(linkNode, lastNode.id, "after"),
			)
		} else {
			// If no nodes exist, replace the container
			dispatch(
				actions_1.EditorActions.replaceContainer(
					__assign(__assign({}, container), { children: [linkNode] }),
				),
			)
		}
		toast({
...
function createHandleCreateTable(params) {
	return (rows, cols) => {
		...
		// Create body rows
		var bodyRows = Array.from({ length: rows }, (_, rowIdx) => {
			var cells = Array.from({ length: cols }, (_, colIdx) => ({
		...
		if (lastNode) {
			dispatch(
				actions_1.EditorActions.insertNode(tableWrapper, lastNode.id, "after"),
			)
		} else {
			// If no nodes exist, replace the container
			dispatch(
				actions_1.EditorActions.replaceContainer(
					__assign(__assign({}, container), { children: [tableWrapper] }),
				),
			)
		}
		toast({
			...
			return __generator(this, (_a) => {
				switch (_a.label) {
					case 0:
						toast = params.toast
						serializeToHtml =
							require("../utils/serialize-to-html").serializeToHtml
