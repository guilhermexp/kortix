    ...
    if (draggedNodeId && draggingNodeId) {
      ...
      if (dropPosition === "left" || dropPosition === "right") {
        ...
        if (isTextNode(draggingNode) && isTextNode(targetNode)) {
          ...
          if (firstIndex > 0) {
            const refNode = container.children[firstIndex - 1];
            if (refNode) {
            ...
            for (let i = 0; i < container.children.length; i++) {
              if (i !== targetRootIndex && i !== draggingRootIndex) {
                const childNode = container.children[i];
                if (childNode) {
        ...
        if (remainingChildren.length === 1) {
          ...
          if (isTargetTheFlexContainer) {
            ...
            if (parentIndex > 0) {
              ...
              const nextNode = container.children[parentIndex + 1];
              if (nextNode) {
            ...
            if (referenceNodeId) {
              ...
              const fallbackChild = remainingChildren[0];
              if (!fallbackChild) return;

              console.log("  No siblings found - using container as reference");
              actions.push(
                EditorActions.insertNode(fallbackChild, container.id, "append"),
              );
              actions.push(
                EditorActions.insertNode(
                  draggingNode,
            ...
            if (isTargetBeforeFlex || isTargetAfterFlex) {
              ...
              const adjacentChild = remainingChildren[0];
              if (!adjacentChild) return;
              ...
              const standardChild = remainingChildren[0];
              if (!standardChild) return;
      ...
      if (draggingNodeAtRoot) {
        const targetNodeAtRoot = container.children.find(
          (n) => n.id === nodeId,
        );
