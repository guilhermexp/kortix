      ...
      {!isMobile && (
        <LayoutGroup>
          <motion.div
            ...
            className="fixed w-full p-4 top-0 left-0 right-0 pointer-events-none z-[60] flex justify-center items-start"
            transition={{
              duration: 0.2,
              ease: [0.4, 0, 0.2, 1],
            }}
          >
            <motion.nav
              ...
              initial={{ scale: 0.95 }}
              style={{
                // Clamp width so it never intrudes into chat area
                width: `min(${menuWidth}px, calc(100vw - ${chatOpen ? chatWidth : 0}px - 64px))`,
              }}
              layout
                ...
                >
                  {!expandedView ? (
                    <motion.div
                      ...
                      className="w-full flex flex-row gap-2 px-2 py-1 justify-center items-center"
                      exit={{
                        opacity: 0,
                        transition: {
                          duration: 0.2,
                        ...
                        {menuItems.map((item, index) => (
                          <Fragment key={item.key}>
                            <motion.button
                              ...
                              className="flex h-8 w-full items-center gap-1.5 rounded-md text-white/80 transition-colors duration-100 hover:text-white cursor-pointer relative px-2"
                            ...
                            >
                              <motion.div
                                ...
                                layout="position"
                              >
                                <item.icon className="duration-200 h-4 w-4 drop-shadow-lg flex-shrink-0" />
                              </motion.div>
                              <span className="drop-shadow-lg pl-2 whitespace-nowrap">
                                {item.text}
                              </span>
                            </motion.button>
                            {index === 0 && (
