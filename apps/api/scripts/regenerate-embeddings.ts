...
const TARGET_TABLE =
	process.argv.find((arg) => arg.startsWith("--table="))?.split("=")[1] || "all"
...
async function regenerateChunkEmbeddings() {
	...
	for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
		...
		try {
			...
			console.log(
				`✓ Processado batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(chunks.length / BATCH_SIZE)} (${processed}/${chunks.length})`,
			)
	...
	for (let i = 0; i < docs.length; i += BATCH_SIZE) {
		...
		try {
			...
			for (const item of successfulEmbeddings) {
				...
				if (updateError) {
					console.error(
						`❌ Erro ao atualizar documento ${item.id}:`,
						updateError,
					)
					errors++
				} else {
			...
			console.log(
				`✓ Processado batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(docs.length / BATCH_SIZE)} (${processed}/${docs.length})`,
			)
...
async function regenerateMemoryEmbeddings() {
	...
	for (let i = 0; i < memories.length; i += BATCH_SIZE) {
		...
		try {
			...
			console.log(
				`✓ Processado batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(memories.length / BATCH_SIZE)} (${processed}/${memories.length})`,
			)
...
async function main() {
	...
	console.log(
		`⚙️  Configurações: batch_size=${BATCH_SIZE}, delay_ms=${DELAY_MS}, table=${TARGET_TABLE}`,
	)
