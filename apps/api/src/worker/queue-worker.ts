...
import { redis, isRedisEnabled } from "../services/queue/redis-client";
...
async function updateDocumentStatus(documentId: string, status: string) {
	try {
		...
		console.warn("[queue-worker] Failed to update status", { documentId, error });
	}
}
...
async function processDocument(job: Job<DocumentJobData>): Promise<void> {
	...
	const processingStatuses = ["fetching", "generating_preview", "extracting", "processing", "indexing"];
	if (existingDoc?.status && processingStatuses.includes(existingDoc.status)) {
		console.log("[queue-worker] Document being processed by DB worker, skipping", {
			jobId: job.id,
			documentId,
			status: existingDoc.status,
		});
	...
	worker.on("failed", (job, error) => {
		...
		if (job?.data?.documentId) {
			supabaseAdmin
				...
				.update({
					status: "failed",
					processing_metadata: sanitizeJson({ error: error.message }) as Record<string, unknown>,
				})
				.eq("id", job.data.documentId)
				.then(() => {
					console.log("[queue-worker] Document marked as failed", { documentId: job.data.documentId });
				});
		}
	});
