...
async function extractPreviewMetadata(url: string): Promise<PreviewMetadata> {
  try {
    ...
    const makeAbsolute = (relativeUrl: string | null | undefined): string | null => {
...
export class AnalysisService {
  ...
  async analyzeWebUrl(url: string, opts?: { useExa?: boolean }) {
    const useExa = typeof opts?.useExa === "boolean" ? opts.useExa : this.defaultUseExa
    ...
    let text = ""
    if (useExa) {
      const contents = await getContentsWithExa([url], { livecrawl: "preferred" })
    ...
    if (useExa) {
      try {
        ...
        const exa = await searchWebWithExa(domain, { limit: 5, includeDomains: [domain] })
        const urls = exa.map((r) => r.url).filter(Boolean) as string[]
        const more = urls.length ? await getContentsWithExa(urls.slice(0, 3), { livecrawl: "preferred" }) : []
        const lines = exa
          .map((r, i) => `(${i + 1}) ${r.title || "Sem título"}\n${r.url || ""}\n${r.snippet || ""}`)
          .join("\n\n")
        const moreText = more.map((m) => m.text || "").join("\n\n").slice(0, 60_000)
  ...
  async analyzeGithubRepository(url: string, githubToken?: string, opts?: { useExa?: boolean }) {
    ...
    const context = collected || `Repositório: ${owner}/${repo} (sem arquivos básicos encontrados)`
    const useExa = typeof opts?.useExa === "boolean" ? opts.useExa : this.defaultUseExa
    let exaContext = ""
    if (useExa) {
      try {
        ...
        const codeLines = code
          .map((r, i) => `(${i + 1}) ${r.title || ""}\n${r.url || ""}\n${(r.text || "").slice(0, 2000)}`)
          .join("\n\n")
        const exa = await searchWebWithExa(`${owner} ${repo} GitHub`, { limit: 5 })
        const lines = exa
          .map((r, i) => `(${i + 1}) ${r.title || "Sem título"}\n${r.url || ""}\n${r.snippet || ""}`)
          .join("\n\n")
  ...
  async analyzeAuto(url: string, title?: string, githubToken?: string, opts?: { useExa?: boolean }) {
    if (this.isYouTube(url)) return this.analyzeYouTubeUrl(url, title)
    if (this.isGithubRepo(url)) return this.analyzeGithubRepository(url, githubToken, opts)
