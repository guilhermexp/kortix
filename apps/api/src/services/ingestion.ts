	...
	try {
		...
		if (result.status === "done") {
				...
				try {
					const chunksToInsert = processed.chunks.map((chunk, index) => {
						try {
							...
							const embeddingValue = (chunk as { embedding?: unknown })
								.embedding
							...
							try {
								...
								console.warn(
									"[ingestion.ts] Failed to extract embedding for chunk",
									{
										index,
							...
							if (!content || content.trim().length === 0) {
								console.warn("[ingestion.ts] Chunk has empty content", {
									index,
								})
							}
							...
							return {
								...
								chunk_index:
									typeof (chunk as { position?: unknown }).position === "number"
										? (chunk as { chunk_index: number }).position
					...
					console.log(
						"[ingestion.ts] Chunks processed, inserting to database:",
						{
							chunkCount: chunksToInsert.length,
							documentId: input.documentId,
							firstChunkSize: chunksToInsert[0]?.content?.length ?? 0,
							hasEmbeddings: chunksToInsert.every(
					...
					console.error(
						"[ingestion.ts] Error during chunk processing/insertion:",
						{
			...
			try {
				...
				const extractionMeta = extraction?.extractionMetadata as
					| Record<string, unknown>
					| null
					| undefined
				const thumbnailUrl =
					typeof extractionMeta?.thumbnailUrl === "string"
				...
				// Priority 3: Try to preserve existing preview from database ONLY if it's NOT SVG
					...
					try {
						...
						const existingPreview =
							typeof currentDoc?.preview_image === "string"
						...
						if (
							existingPreview &&
							!existingPreview.includes("data:image/svg+xml")
						) {
							console.log(
								"[ingestion.ts] Preserving existing preview_image from database (not SVG)",
								{
									documentId: input.documentId,
							...
							console.log(
								"[ingestion.ts] Skipping SVG placeholder from database",
								{
									documentId: input.documentId,
								},
							)
						...
						console.warn(
							"[ingestion.ts] Failed to fetch current document for preview preservation",
							{
								documentId: input.documentId,
				...
				if (previewImage?.includes("data:image/svg+xml")) {
					console.warn(
						"[ingestion.ts] Blocked SVG placeholder from being saved",
						{
							documentId: input.documentId,
						},
					)
				...
				const processedMetadata = isJsonRecord(processed?.metadata)
					...
					: null
				const extractionMetadataForMerge = isJsonRecord(
				...
				if (Array.isArray(extraction?.images) && extraction.images.length > 0) {
					...
					extractionRaw &&
					Array.isArray(extractionRaw.images) &&
					extractionRaw.images.length > 0
				) {
					imagesToExtract = extractionRaw.images
				} else if (
					mergedExtractionRaw &&
					Array.isArray(mergedExtractionRaw.images) &&
					mergedExtractionRaw.images.length > 0
				) {
				...
				if (
					!resolvedTitle ||
					resolvedTitle.toLowerCase() === "untitled" ||
				...
				if (sanitizedSummary && sanitizedSummary.trim().length > 0) {
					...
					try {
						...
						console.error(
							"[ingestion.ts] Failed to upsert auto-summary memory",
							{
								documentId: input.documentId,
