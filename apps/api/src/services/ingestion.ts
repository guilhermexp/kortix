...
/**
 * Sanitize JSON object to remove invalid Unicode surrogates that cause PostgreSQL errors
 * This handles orphaned high/low surrogates that break JSON serialization
 */
function sanitizeJsonForPostgres(obj: JsonRecord | null): JsonRecord | null {
  if (!obj) return null;
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (value === null || value === undefined) {
      ...
      // Remove orphaned Unicode surrogates that cause "Unicode low surrogate must follow a high surrogate" errors
      try {
        // Attempt to JSON.stringify and parse to detect invalid surrogates
        JSON.stringify(value);
        sanitized[key] = value;
      } catch {
        // If it contains bad surrogates, remove them
        sanitized[key] = value
      ...
      sanitized[key] = sanitizeJsonForPostgres(value as JsonRecord);
    } else if (Array.isArray(value)) {
      // Handle arrays of objects
      sanitized[key] = value.map((item) => {
        if (typeof item === "object" && item !== null && !Array.isArray(item)) {
          return sanitizeJsonForPostgres(item as JsonRecord);
        }
...
export async function processDocument(input: ProcessDocumentInput) {
  ...
  try {
    ...
    const documentUpdate = {
      ...
      source: extraction.source ?? document.source ?? null,
      metadata: sanitizeJsonForPostgres(mergedMetadata),
      processing_metadata: sanitizeJsonForPostgres(processingMetadata),
      raw: sanitizeJsonForPostgres(mergedRaw),
