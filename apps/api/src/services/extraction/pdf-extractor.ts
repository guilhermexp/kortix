...
export class PDFExtractor extends BaseService implements IPDFExtractor {
	...
	constructor(options?: { ocrEnabled?: boolean; ocrProvider?: 'replicate' | 'gemini' }) {
		super('PDFExtractor')
	...
	async extract(input: ExtractionInput): Promise<ExtractionResult> {
		...
		if (!input.fileBuffer) {
			throw this.createError('MISSING_FILE', 'File buffer is required for PDF extraction')
		}
	...
	async validateInput(input: ExtractionInput): Promise<void> {
		if (!input.fileBuffer && !input.url) {
			throw this.createError('VALIDATION_ERROR', 'File buffer or URL is required')
		}
		...
		if (input.fileBuffer) {
			...
			if (input.fileBuffer.length > maxSize) {
				throw this.createError('FILE_TOO_LARGE', `PDF file exceeds maximum size of 50MB`)
			}
		}
	}
	 ...
	 * ```
	 */
	async extractFromPDF(buffer: Buffer, options?: PDFOptions): Promise<ExtractionResult> {
		this.assertInitialized()
		...
		try {
			...
			if (options?.useOCR && (isScanned || this.ocrEnabled)) {
				try {
					...
					this.logger.warn('OCR extraction failed, falling back to text extraction', {
	...
	async extractWithOCR(buffer: Buffer, options?: OCROptions): Promise<string> {
		...
		throw this.createError('INVALID_OCR_PROVIDER', `Unknown OCR provider: ${provider}`)
	}
	...
	private async extractTextFromPDF(buffer: Buffer): Promise<string> {
		try {
			...
			throw this.createError('PDF_EXTRACTION_FAILED', 'Failed to extract text from PDF')
		}
	}
