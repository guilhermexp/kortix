...
describe("YouTubeExtractor", () => {
	...
	describe("Video Metadata Extraction", () => {
		...
		it("should extract thumbnail URLs in different qualities", async () => {
			...
			expect(result.success).toBe(true)
			expect(result.data?.metadata.thumbnails?.maxres).toContain('maxresdefault')
		})
	})
	...
	describe("Transcript Extraction", () => {
		it("should extract transcript with timestamps", async () => {
			...
			const mockTranscript = [
				...
				{ start: 7, duration: 4, text: "A full commitment's what I'm thinking of" },
			]

			const mockResult: ExtractionResult = {
				success: true,
				data: {
					content: mockTranscript.map(t => `[${t.start}s] ${t.text}`).join('\n'),
					metadata: {
						videoId: 'dQw4w9WgXcQ',
						title: 'Rick Astley - Never Gonna Give You Up',
		...
		it("should fallback to OCR when transcript unavailable", async () => {
			...
			const mockOCRResult = [
				...
				{ text: "This is fallback content when transcript fails", confidence: 0.78 },
			]
		...
		it("should try multiple language fallbacks for transcripts", async () => {
			...
			extractSpy
				...
				.mockResolvedValue({
					transcript: [{ start: 0, duration: 5, text: "English transcript fallback" }],
					language: 'en',
					available: true,
				})
	...
	describe("Content Quality Assessment", () => {
		it("should assess content quality based on transcript length", async () => {
			...
			const longTranscript = Array(100).fill(null).map((_, i) => ({
				start: i * 10,
				duration: 10,
