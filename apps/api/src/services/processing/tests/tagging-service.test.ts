...
describe("TaggingService", () => {
	...
	describe("Basic Tagging", () => {
		it("should generate tags for text content", async () => {
			const text = "This document discusses artificial intelligence, machine learning, and data science techniques for natural language processing."

			const mockTags: TagWithMetadata[] = [
				{ tag: 'artificial intelligence', type: 'topic', confidence: 0.95, category: 'Technology' },
				{ tag: 'machine learning', type: 'topic', confidence: 0.92, category: 'Technology' },
				{ tag: 'data science', type: 'topic', confidence: 0.88, category: 'Technology' },
				{ tag: 'natural language processing', type: 'concept', confidence: 0.85, category: 'AI' },
			]
		...
		it("should respect maximum tag limit", async () => {
			const text = "Document with many potential tags covering various topics and subjects"
			
			const manyTags: TagWithMetadata[] = Array(15).fill(null).map((_, i) => ({
				tag: `tag-${i}`,
				type: 'keyword' as any,
			...
			expect(result.success).toBe(true)
			expect(result.data?.tags!.length).toBeLessThanOrEqual(mockOptions.maxTags!)
			// Should return highest confidence tags
			expect(result.data?.tags![0].confidence).toBeGreaterThanOrEqual(result.data?.tags!.at(-1)!.confidence)
		})
		...
		it("should filter tags by minimum confidence", async () => {
			...
			const mixedConfidenceTags: TagWithMetadata[] = [
				{ tag: 'high-confidence', type: 'topic', confidence: 0.95, category: 'Test' },
				{ tag: 'medium-confidence', type: 'topic', confidence: 0.75, category: 'Test' },
				{ tag: 'low-confidence', type: 'topic', confidence: 0.5, category: 'Test' },
				{ tag: 'very-low-confidence', type: 'topic', confidence: 0.3, category: 'Test' },
			]
			...
			const lowConfidenceTags = result.data?.tags?.filter(tag => tag.confidence < mockOptions.minConfidence!)
	...
	describe("Tag Categories", () => {
		it("should include tag categories when enabled", async () => {
			...
			const categorizedTags: TagWithMetadata[] = [
				{ tag: 'programming', type: 'topic', confidence: 0.9, category: 'Technology' },
				{ tag: 'software development', type: 'topic', confidence: 0.85, category: 'Technology' },
				{ tag: 'JavaScript', type: 'entity', confidence: 0.8, category: 'Programming Languages' },
			]
		...
		it("should group tags by category", async () => {
			...
			const multiCategoryTags: TagWithMetadata[] = [
				...
				{ tag: 'machine learning', type: 'topic', confidence: 0.85, category: 'Technology' },
				{ tag: 'finance', type: 'topic', confidence: 0.8, category: 'Business' },
				{ tag: 'trading', type: 'topic', confidence: 0.75, category: 'Business' },
			]
			...
			const categoryGroups = service.groupTagsByCategory(result.data?.tags || [])
	...
	describe("Tag Types", () => {
		it("should generate different types of tags", async () => {
			const text = "The company Tesla, founded by Elon Musk, is developing autonomous vehicles using artificial intelligence and neural networks."
			
			const typedTags: TagWithMetadata[] = [
				{ tag: 'Tesla', type: 'entity', confidence: 0.95, category: 'Companies' },
				{ tag: 'Elon Musk', type: 'entity', confidence: 0.9, category: 'People' },
				{ tag: 'autonomous vehicles', type: 'concept', confidence: 0.85, category: 'Technology' },
				{ tag: 'artificial intelligence', type: 'topic', confidence: 0.8, category: 'Technology' },
				{ tag: 'neural networks', type: 'concept', confidence: 0.75, category: 'AI' },
			]
		...
		it("should handle auto-tagging when enabled", async () => {
			const text = "This document contains keywords like document management, file organization, and data storage solutions."
	...
	describe("Performance", () => {
		it("should handle concurrent tagging requests", async () => {
			const texts = Array(5).fill(null).map((_, i) => `Text ${i} for tagging`)
	...
	describe("Tag Validation", () => {
		it("should validate tag quality", async () => {
			...
			const qualityTags: TagWithMetadata[] = [
				...
				{ tag: 'document', type: 'keyword', confidence: 0.85, category: 'General' },
				{ tag: 'a', type: 'keyword', confidence: 0.95, category: 'General' }, // Too short
				{ tag: 'verylongtagthatexceedslengthlimit', type: 'keyword', confidence: 0.8, category: 'General' }, // Too long
			]
			...
			const validTags = result.data?.tags?.filter(tag => 
				tag.tag.length >= 2 && tag.tag.length <= 20
			) || []
	...
	describe("Edge Cases", () => {
		it("should handle texts with special characters", async () => {
			const specialText = "Document with émojis 🎯 and spëciäl chårs & symbols @#$%"
