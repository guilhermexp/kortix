	...
	return createSdkMcpServer({
		...
		tools: [
			tool(
				...
				{
					query: z.string().min(1).describe("Search query text - can be keywords, questions, or topics"),
					limit: z.number().min(1).max(50).default(20).describe("Maximum number of results to return"),
					includeSummary: z.boolean().default(true).describe("Include document summaries in results"),
					includeFullDocs: z.boolean().default(false).describe("Include full document content (use only when specifically needed, as it increases response size)"),
					containerTags: z.array(z.string()).optional().describe("Filter by project/container tags"),
					scopedDocumentIds: z.array(z.string()).optional().describe("Limit search to specific document IDs"),
				},
				async ({
					query,
					limit,
					...
					if (cached) {
						...
						console.log(`[searchDatabase] Cache hit for query "${query}" (${duration}ms)`)
						return {
							content: [
								{
									type: "text",
					...
					try {
						...
						const duration = Date.now() - startTime
						console.log(`[searchDatabase] Found ${response.total} results (${duration}ms)`)
