...
/**
 * Sanitize a string to remove orphaned Unicode surrogates that cause PostgreSQL JSON errors
 * This handles the "Unicode low surrogate must follow a high surrogate" error
 * Should be applied to any extracted text/metadata that will be stored in JSON columns
 */
function sanitizeUnicodeString(value: string): string {
  ...
  try {
    ...
    // If it fails, remove orphaned surrogates
    return value
      .replace(/[\uD800-\uDBFF](?![\uDC00-\uDFFF])/g, "") // Remove orphaned high surrogates
      .replace(/(?<![\uD800-\uDBFF])[\uDC00-\uDFFF]/g, ""); // Remove orphaned low surrogates
  }
}

/**
 * Recursively sanitize an object to remove orphaned Unicode surrogates
 * Applied to raw metadata objects from extraction
 */
function sanitizeUnicodeObject(
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (value === null || value === undefined) {
      ...
      sanitized[key] = sanitizeUnicodeObject(value as Record<string, unknown>);
    } else if (Array.isArray(value)) {
      sanitized[key] = value.map((item) => {
        if (typeof item === "string") {
          return sanitizeUnicodeString(item);
        } else if (
          typeof item === "object" &&
          item !== null &&
          !Array.isArray(item)
        ) {
          return sanitizeUnicodeObject(item as Record<string, unknown>);
        }
        return item;
      });
    } else {
...
function extractMetaTags(html: string): {
  ...
  if (ogImageMatch?.[1])
    result.ogImage = sanitizeUnicodeString(ogImageMatch[1]);
  ...
  if (twitterImageMatch?.[1])
    result.twitterImage = sanitizeUnicodeString(twitterImageMatch[1]);
  ...
  if (ogTitleMatch?.[1])
    result.ogTitle = sanitizeUnicodeString(ogTitleMatch[1]);
  ...
  if (ogDescriptionMatch?.[1])
    result.ogDescription = sanitizeUnicodeString(ogDescriptionMatch[1]);
  ...
  if (faviconMatch?.[1])
    result.favicon = sanitizeUnicodeString(faviconMatch[1]);
          ...
          try {
            ...
            if (htmlResponse.ok) {
              ...
              if (!ogImage) {
                ...
                ogImage = await extractPreviewImageWithGemini(
                  html,
                  probableUrl,
                );
                if (ogImage) {
