...
let orchestratorServiceInstance: Awaited<ReturnType<typeof createIngestionOrchestrator>> | null = null
let extractorServiceInstance: Awaited<ReturnType<typeof createDocumentExtractorService>> | null = null
let processorServiceInstance: Awaited<ReturnType<typeof createDocumentProcessorService>> | null = null
let previewServiceInstance: Awaited<ReturnType<typeof createPreviewGeneratorService>> | null = null
...
export async function getOrchestrator() {
  if (!orchestratorServiceInstance) {
    ...
    processorServiceInstance = createDocumentProcessorService({
      ...
      embedding: { enabled: true, provider: "voyage", batchSize: 10, useCache: true },
    ...
    orchestratorServiceInstance = createIngestionOrchestrator({ timeout: 60000, circuitBreaker: { enabled: true } })
    await Promise.all([
