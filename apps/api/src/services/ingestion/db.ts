...
export async function updateDocumentStatus(documentId: string, status: string, extra?: JsonRecord) {
...
export async function upsertAutoSummaryMemory(params: {
  ...
  const { documentId, organizationId, userId, spaceId, summary, metadata } = params
  ...
  if (existing && existing.length > 0) {
    ...
    const nextVersion = typeof current.version === "number" && Number.isFinite(current.version) ? current.version + 1 : 1
