...
describe("Preview Service Components", () => {
	...
	describe("SVGGenerator", () => {
		...
		it("should generate SVG preview from document", async () => {
			...
			const mockSVG = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300"><rect width="400" height="300" fill="#f0f0f0"/><text x="200" y="150" text-anchor="middle">Test Document</text></svg>'
		...
		it("should support different themes", async () => {
			...
			for (const theme of themes) {
				...
				const result = await generator.generatePreview(documentInput, { theme: theme as any })
		...
		it("should handle custom dimensions", async () => {
			...
			const result = await generator.generatePreview(documentInput, customOptions)
	...
	describe("FaviconExtractor", () => {
		...
		it("should extract favicon from URL", async () => {
			...
			expect(result.data?.favicons![0].url).toBe('https://example.com/favicon.ico')
		...
		it("should handle multiple favicon sizes", async () => {
			...
			const mockFavicons = {
				favicons: [
					{ url: 'https://example.com/favicon-16x16.png', format: 'png', size: 16 },
					{ url: 'https://example.com/favicon-32x32.png', format: 'png', size: 32 },
		...
		it("should generate default favicon when none found", async () => {
			...
			const result = await extractor.extractFavicon(url, { generateDefault: true })
	...
	describe("Integration Scenarios", () => {
		it("should work together as fallback chain", async () => {
			...
			const imageSpy = vi.spyOn(imageExtractor as any, 'extractImagesFromDocument')
			...
			const faviconExtractor = new FaviconExtractor({ includeMultipleSizes: false })
			const faviconSpy = vi.spyOn(faviconExtractor as any, 'fetchFaviconFromURL')
			...
			const faviconResult = await faviconExtractor.extractFavicon(documentInput.url!)
