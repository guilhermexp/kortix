...
describe("PreviewGeneratorService", () => {
	...
	describe("SVG Preview Generation", () => {
		it("should generate SVG preview from document content", async () => {
			...
			const mockSVGPreview = {
				previewType: 'svg',
				content: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300"><rect width="400" height="300" fill="#f0f0f0"/><text x="200" y="150" text-anchor="middle" font-family="Arial" font-size="16">Document Title</text></svg>',
				metadata: {
		...
		it("should support different SVG themes", async () => {
			...
			for (const theme of themes) {
				...
				const result = await service.generateSVGPreview(documentInput, { theme: theme as any })
	...
	describe("Performance Optimization", () => {
		...
		it("should implement timeout handling", async () => {
			...
			const result = await service.generatePreview(documentInput, { timeoutMs: 1000 })
