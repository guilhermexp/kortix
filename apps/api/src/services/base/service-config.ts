...
export function getDefaultExtractorConfig(): ExtractorServiceConfig {
	return {
		...
		youtube: {
			...
			preferredLanguages: ConfigLoader.getArray("YOUTUBE_LANGUAGES", [
				"en",
				"en-US",
			]),
...
export function getDefaultProcessorConfig(): ProcessorServiceConfig {
	return {
		...
		embedding: {
			provider:
				(process.env.EMBEDDING_PROVIDER as "gemini" | "openai" | "hybrid") ||
				"gemini",
		...
		summarization: {
			...
			provider:
				(process.env.SUMMARIZATION_PROVIDER as
					| "openrouter"
					| "gemini"
					| "claude") || "openrouter",
		...
		tagging: {
			...
			provider:
				(process.env.TAGGING_PROVIDER as "openrouter" | "gemini") ||
				"openrouter",
...
export function getDefaultPreviewConfig(): PreviewServiceConfig {
	return {
		imageExtractionEnabled: ConfigLoader.getBoolean(
			"PREVIEW_IMAGE_EXTRACTION",
			true,
		),
		svgGenerationEnabled: ConfigLoader.getBoolean(
			"PREVIEW_SVG_GENERATION",
			true,
		),
		faviconFallbackEnabled: ConfigLoader.getBoolean(
			"PREVIEW_FAVICON_FALLBACK",
			true,
		),
...
export function getDefaultOrchestratorConfig(): OrchestratorServiceConfig {
	return {
		circuitBreaker: {
			...
			resetTimeout: ConfigLoader.getNumber(
				"CIRCUIT_BREAKER_RESET_TIMEOUT",
				60000,
			), // 1 minute
			monitoringWindow: ConfigLoader.getNumber(
...
export function validateExtractorConfig(config: ExtractorServiceConfig): void {
	...
	if (
		config.repository?.timeout !== undefined &&
		config.repository.timeout < 0
	) {
...
export function validateProcessorConfig(config: ProcessorServiceConfig): void {
	// Validate chunking
	if (config.chunking) {
		const { defaultChunkSize, defaultOverlap, minChunkSize, maxChunkSize } =
			config.chunking
...
export function validateOrchestratorConfig(
	...
	if (config.circuitBreaker) {
		const { failureThreshold, resetTimeout, monitoringWindow } =
