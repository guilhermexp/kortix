...
describe("API Smoke - Auth gating and basic endpoints", () => {
  ...
  app.post("/v3/search", zValidator("json", z.object({ q: z.string() })), (c) => c.json({ ok: true }))
  ...
  it("/api/auth/sign-in inválido retorna 400", async () => {
    const res = await app.request("/api/auth/sign-in", { method: "POST", body: JSON.stringify({}) })
  ...
  it("/v3/search requer autenticação", async () => {
    const res = await app.request("/v3/search", { method: "POST", body: JSON.stringify({ q: "test" }) })
    expect(res.status).toBe(401)
  })

  it("/v3/documents requer autenticação", async () => {
    const res = await app.request("/v3/documents", { method: "POST", body: JSON.stringify({}) })
    expect(res.status).toBe(401)
  })

  it("/v3/connections/list requer autenticação", async () => {
    const res = await app.request("/v3/connections/list", { method: "POST", body: JSON.stringify({}) })
    expect(res.status).toBe(401)
  })
  ...
  it("/chat requer autenticação", async () => {
    const res = await app.request("/chat", { method: "POST", body: JSON.stringify({}) })
    expect(res.status).toBe(401)
  })

  it("/chat/v2 requer autenticação", async () => {
    const res = await app.request("/chat/v2", { method: "POST", body: JSON.stringify({}) })
    expect(res.status).toBe(401)
  })
