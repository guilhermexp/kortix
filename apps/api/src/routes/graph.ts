...
function extractSpaces(doc: RawDocumentRow): string[] {
	...
	const relationSpaces: string[] = []
	if (doc.spaces?.container_tag) {
		const tag = doc.spaces.container_tag.trim()
		if (tag.length > 0) {
			relationSpaces.push(tag)
		}
	}
...
export async function handleGraphConnections({
	...
	try {
		let query = client
			.from("documents")
			.select(
				"id, title, summary, summary_embedding, metadata, space_id, spaces(container_tag)",
			)
			.eq("org_id", orgId)
