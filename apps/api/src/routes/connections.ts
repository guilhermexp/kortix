...
export async function listConnections(client: SupabaseClient, organizationId: string, containerTags?: string[]) {
  const { data, error } = await client
    .from("connections")
    .select("id, provider, email, document_limit, metadata, expires_at, created_at, container_tags")
  ...
  const filtered = (data ?? []).filter((connection) => {
    ...
    const tags = Array.isArray(connection.container_tags) ? connection.container_tags : []
...
export async function getConnection(client: SupabaseClient, organizationId: string, connectionId: string) {
  const { data, error } = await client
    .from("connections")
    .select("id, provider, email, document_limit, metadata, expires_at, created_at")
    .eq("org_id", organizationId)
    .eq("id", connectionId)
    .maybeSingle()
...
export async function deleteConnection(client: SupabaseClient, organizationId: string, connectionId: string) {
