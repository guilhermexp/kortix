...
function mapSpaceToProject(space: any) {
...
export async function listProjects(client: SupabaseClient, organizationId: string) {
  const { data, error } = await client
    .from("spaces")
    .select("id, container_tag, name, is_experimental, created_at, updated_at, documents_to_spaces(document_id)")
  ...
  if (parsed.action === "move") {
    ...
    if (links && links.length > 0) {
      const inserts = links.map((link) => ({
        ...
        space_id: parsed.targetProjectId!,
      }))
