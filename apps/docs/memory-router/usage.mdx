...
You'll need:
1. Um token gerado no painel self-hosted (`http://localhost:3000` → **Configurações → API Keys**)
2. A chave da API do seu provedor de LLM

## Basic Setup

<Steps>
  <Step title="Get Your API Keys">
    **Token do Supermemory:**
    1. Faça login em `http://localhost:3000`
    2. Abra **Configurações → API Keys**
    3. Gere e copie o token de serviço
  ...
  <Step title="Update Your Base URL">
    Prepend `http://localhost:4000/v3/` to your provider's URL:

    ```
    http://localhost:4000/v3/[PROVIDER_URL]
    ```
  </Step>
...
```text OpenAI
http://localhost:4000/v3/https://api.openai.com/v1/
```

```text Anthropic
http://localhost:4000/v3/https://api.anthropic.com/v1/
```

```text Google Gemini
http://localhost:4000/v3/https://generativelanguage.googleapis.com/v1beta/openai/
```

```text Groq
http://localhost:4000/v3/https://api.groq.com/openai/v1/
```
...
<Tabs>
  <Tab title="Python">
    ...
    client = OpenAI(
        ...
        base_url="http://localhost:4000/v3/https://api.openai.com/v1/",
        default_headers={
  ...
  <Tab title="cURL">
    ```bash
    curl -X POST "http://localhost:4000/v3/https://api.openai.com/v1/chat/completions" \
...
client = OpenAI(
    api_key="YOUR_OPENAI_API_KEY",
    base_url="http://localhost:4000/v3/https://api.openai.com/v1/chat/completions?userId=user123"
)
...
const client = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY,
  baseURL: 'http://localhost:4000/v3/https://api.openai.com/v1/chat/completions?userId=user123'
});
...
```bash cURL
curl -X POST "http://localhost:4000/v3/https://api.openai.com/v1/chat/completions?userId=user123" \
  -H "Authorization: Bearer YOUR_OPENAI_API_KEY" \
  -H "x-supermemory-api-key: YOUR_SUPERMEMORY_API_KEY" \
  -H "Content-Type: application/json" \
