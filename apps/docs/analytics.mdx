...
const usage = await fetch('http://localhost:4000/v3/analytics/usage?period=24h', {
  headers: {
...
response = requests.get(
    'http://localhost:4000/v3/analytics/usage',
    params={'period': '7d'},
...
curl -X GET "http://localhost:4000/v3/analytics/usage?from=2024-01-01T00:00:00Z&to=2024-01-31T23:59:59Z" \
...
const errors = await fetch('http://localhost:4000/v3/analytics/errors?period=24h', {
  headers: {
    'Authorization': `Bearer ${SUPERMEMORY_API_KEY}`
  }
});
...
response = requests.get(
    'http://localhost:4000/v3/analytics/errors?period=1h',
    headers={'Authorization': f'Bearer {SUPERMEMORY_API_KEY}'}
)
...
curl -X GET "http://localhost:4000/v3/analytics/errors?period=7d" \
  -H "Authorization: Bearer $SUPERMEMORY_API_KEY"
```
...
const logs = await fetch('http://localhost:4000/v3/analytics/logs?period=1h&statusCode=500', {
  headers: {
    'Authorization': `Bearer ${SUPERMEMORY_API_KEY}`
  }
});
...
response = requests.get(
    'http://localhost:4000/v3/analytics/logs',
    params={
        'keyId': 'key_abc123',
        'period': '24h'
    },
...
curl -X GET "http://localhost:4000/v3/analytics/logs?period=1h&limit=50" \
  -H "Authorization: Bearer $SUPERMEMORY_API_KEY"
```
