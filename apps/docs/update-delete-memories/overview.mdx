...
```bash cURL
curl -X PATCH "http://localhost:4000/v3/documents/memory_id_123" \
...
```bash cURL
curl -X DELETE "http://localhost:4000/v3/documents/memory_id_123" \
  -H "Authorization: Bearer $SUPERMEMORY_API_KEY"
...
```bash cURL
curl -X DELETE "http://localhost:4000/v3/documents/bulk" \
  -H "Authorization: Bearer $SUPERMEMORY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "ids": [
...
curl -X PATCH "http://localhost:4000/v3/documents/memory_id" \
  -H "Authorization: Bearer $SUPERMEMORY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "metadata": {
...
for ((i=0; i<${#MEMORY_IDS[@]}; i+=BATCH_SIZE)); do
    ...
    response=$(curl -s -X DELETE \
      "http://localhost:4000/v3/documents/bulk" \
      -H "Authorization: Bearer $SUPERMEMORY_API_KEY" \
      -H "Content-Type: application/json" \
      -d "{\"ids\": $batch_json}")
