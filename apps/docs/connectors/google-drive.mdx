...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    curl -X POST "http://localhost:4000/v3/connections/google-drive" \
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    curl -X POST "http://localhost:4000/v3/connections/list" \
      -H "Authorization: Bearer $SUPERMEMORY_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "containerTags": ["user-123", "gdrive-sync"],
    ...
    # List synced documents
    curl -X POST "http://localhost:4000/v3/documents/list" \
      -H "Authorization: Bearer $SUPERMEMORY_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "containerTags": ["user-123", "gdrive-sync"],
...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    # Delete by connection ID
    curl -X DELETE "http://localhost:4000/v3/connections/connection_id_123" \
      -H "Authorization: Bearer $SUPERMEMORY_API_KEY"

    # Response: {"id": "connection_id_123", "provider": "google-drive"}

    # Delete by provider and container tags
    curl -X DELETE "http://localhost:4000/v3/connections/google-drive" \
      -H "Authorization: Bearer $SUPERMEMORY_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "containerTags": ["user-123"]
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    curl -X POST "http://localhost:4000/v3/connections/google-drive/import" \
      -H "Authorization: Bearer $SUPERMEMORY_API_KEY"
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    curl -X PATCH "http://localhost:4000/v3/settings" \
      -H "Authorization: Bearer $SUPERMEMORY_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "googleDriveCustomKeyEnabled": true,
    ...
    # Get current settings
    curl -X GET "http://localhost:4000/v3/settings" \
      -H "Authorization: Bearer $SUPERMEMORY_API_KEY"
    ```
  </Tab>
</Tabs>
