...
```bash cURL
curl -X POST "http://localhost:4000/v3/connections/onedrive" \
...
<CodeGroup>
  ...
  ```bash cURL
    # Trigger manual sync
    curl -X POST "http://localhost:4000/v3/connections/onedrive/import" \
      -H "Authorization: Bearer $SUPERMEMORY_API_KEY" \
      -H "Content-Type: application/json" \
...
# Delete by provider and container tags
curl -X DELETE "http://localhost:4000/v3/connections/onedrive" \
  -H "Authorization: Bearer $SUPERMEMORY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"containerTags": ["user-123"]}'
```
