...
const response = await fetch('http://localhost:4000/v3/documents/processing', {
  headers: {
...
response = requests.get(
    'http://localhost:4000/v3/documents/processing',
...
```bash
curl -X GET "http://localhost:4000/v3/documents/processing" \
...
```bash
curl -X GET "http://localhost:4000/v3/documents/doc_abc123" \
  -H "Authorization: Bearer $SUPERMEMORY_API_KEY"
```
