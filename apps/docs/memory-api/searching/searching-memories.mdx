...
To search through your memories today, use `/v3/search` (documents) or `/v3/search/hybrid`. The `/v4/search` (memories) endpoint is planned and not available in v1.2.
...
curl -X POST http://localhost:4000/v3/search \
  ...
  -d '{"q":"machine learning concepts","limit":10}'
```
...
## Including Forgotten Memories (Planned)

By default, the search API excludes memories that have been marked as forgotten or have passed their expiration date. A future `/v4/search` endpoint will support including these in results via `include.forgottenMemories`.
...
Explore more advanced features in the [Search Overview](/search/overview) and [Document Search Examples](/search/examples/document-search).
