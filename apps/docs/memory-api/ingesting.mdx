...
Você pode acompanhar o progresso direto no painel self-hosted (`http://localhost:3000`), na aba **Documentos**, que mostra como cada arquivo gera memórias e chunks relacionados.
...
```bash cURL
curl http://localhost:4000/v3/documents \
...
```bash cURL
curl http://localhost:4000/v3/documents/file \
  -H "Authorization: Bearer $SUPERMEMORY_API_KEY" \
  -F "file=@document.pdf" \
...
const response = await fetch('http://localhost:4000/v3/documents/file', {
  method: 'POST',
  headers: {
...
```bash
curl -X PATCH "http://localhost:4000/v3/documents/abc123" \
  -H "Authorization: Bearer $SUPERMEMORY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
<Note>
Pronto para começar? Gere um token em **Configurações → API Keys** dentro do painel self-hosted e faça suas chamadas para `http://localhost:4000/v3/documents`.
</Note>
