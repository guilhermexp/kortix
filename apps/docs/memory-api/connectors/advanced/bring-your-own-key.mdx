...
```
http://localhost:4000/v3/connections/auth/callback/{provider}
```

For example, if you are using Google Drive, the redirect URL would be:

```
http://localhost:4000/v3/connections/auth/callback/google-drive
```
...
# Example: Configure Google Drive custom OAuth credentials
curl --request PATCH \
  --url http://localhost:4000/v3/settings \
