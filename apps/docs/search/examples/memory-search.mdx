---
title: "Memories Search (/v4/search) – Coming Soon"
description: "Planned endpoint optimized for chatbots and conversational AI"
---


<Warning>
The `/v4/search` endpoint is not available in v1.2. The examples below are illustrative. Use `/v3/search` or `/v3/search/hybrid` meanwhile for production.
</Warning>

Memories search (`POST /v4/search`) will provide minimal-latency search optimized for real-time interactions. This endpoint prioritizes speed over extensive control, making it perfect for chatbots, Q&A systems, and any application where users expect immediate responses.
...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    # Not available in v1.2. Suggested alternative:
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    # Use /v3/search or apply filtering via containerTags when using documents search
    ```
  </Tab>
</Tabs>
