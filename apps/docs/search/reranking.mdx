...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    # Not available in v1.2. Consider /v3/search/hybrid with rerank enabled
    ```
  </Tab>
</Tabs>
...
<Warning>
The `/v4/search` (Memories Search) endpoint referenced below is not available in v1.2. Use `/v3/search` or `/v3/search/hybrid` instead. Memory-specific examples are illustrative for a future release.
</Warning>
