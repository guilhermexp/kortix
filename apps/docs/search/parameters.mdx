...
Complete parameter reference for document search and planned memory search.
...
## Memory Search Parameters (POST `/v4/search`) – Coming Soon

<Warning>
The `/v4/search` endpoint is not available in v1.2. The parameter reference below is forward-looking.
</Warning>
