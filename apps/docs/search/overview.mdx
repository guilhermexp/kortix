...
<CardGroup cols={2}>
  ...
  <Card title="Memories Search (Coming Soon)" icon="zap" href="/search/examples/memory-search">
    **POST /v4/search** — Not available in v1.2.

    Planned endpoint optimized for chatbots and conversational AI. For now, use `/v3/search` or `/v3/search/hybrid`.
  </Card>
</CardGroup>

<Warning>
The `/v4/search` endpoint (Memories Search) is not implemented in this version. Please use `/v3/search` (documents) or `/v3/search/hybrid` (FTS + vector + RRF) instead.
</Warning>
...
### Memories Search (`/v4/search`) — Coming Soon
**Planned** search through user memories:
...
<Info>
  This endpoint is planned. Until it ships, prefer `/v3/search` or `/v3/search/hybrid` for conversational use cases.
</Info>
...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    # Not available in v1.2. Use /v3/search or /v3/search/hybrid instead
...
The `/v4/search` endpoint will search through and return memories (planned).
