...
Supermemory supports filtering search results using container tags, metadata conditions, and advanced filtering techniques for `/v3/search`. The `/v4/search` (memories) endpoint is planned and not available in v1.2.
...
### Memory Search (v4/search) – Coming Soon
...
<Note>
**Container Tag Differences** (planned):
- `/v3/search` uses `containerTags` (plural array) with exact array matching
- `/v4/search` will use `containerTag` (singular string) for single tag filtering
