...
async function testProviderSwitch() {
  ...
  async function* promptGenerator() {
    yield {
      ...
      message: {
        role: "user" as const,
        content: [{ type: "text" as const, text: "Diga apenas 'olá' em uma palavra" }]
      }
    }
  }
  ...
  try {
    ...
    for await (const event of agentIterator) {
      ...
      if (event && typeof event === 'object') {
        ...
        if (typed.type === 'system') {
          console.log(`[Evento ${eventCount}] SYSTEM:`, JSON.stringify(typed, null, 2))
          ...
          console.log(`[Evento ${eventCount}] ERROR:`, JSON.stringify(typed, null, 2))
        }
      }
    }
    ...
    if (responseText) {
      console.log("✅ SUCESSO: SDK conseguiu fazer chamada com base URL customizada!")
