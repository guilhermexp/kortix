	...
	for (const doc of docs) {
		try {
			...
			if (raw.images && Array.isArray(raw.images) && raw.images.length > 0) {
				console.log(
					`✅ ${doc.title} - already has root images (${raw.images.length} images)`,
				)
				skipped++
				continue
			}
			...
			if (
				!extraction ||
				!Array.isArray(extraction.images) ||
			...
			console.log(
				`✅ Fixed ${doc.title} - moved ${extraction.images.length} images to root`,
			)
			fixed++
		} catch (err) {
